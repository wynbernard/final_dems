...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // New Pick Up Point Information fields
    ...
    if ($stmt->execute()) {
        ...
        // Handle family members if registration type is family
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'DEMS Pre-Registration - Account Details';
            $mailBody = "<p>Dear " . htmlspecialchars($f_name) . ",</p>" .
                "<p>Your pre-registration has been completed on the DEMS system. Please find your login details below:</p>" .
            ...
            $mail->Body = $mailBody;
            $mail->AltBody = "DEMS pre-registration completed. Email: $email\nPassword: $password";

            $mail->send();
        } catch (Exception $e) {
            error_log('Pre-registration email failed: ' . $e->getMessage());
        }

        echo json_encode(['success' => true, 'message' => 'Pre-registration successful!', 'keep_modal_open' => true]);
    } else {
