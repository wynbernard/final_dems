...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (empty($disaster_id) || empty($disaster_type) || empty($disaster_date)) {
	...
	$sql = "UPDATE disaster_table SET disaster_name = ?, date = ? WHERE disaster_id = ?";
	$stmt = mysqli_prepare($conn, $sql);
	mysqli_stmt_bind_param($stmt, "ssi", $disaster_type, $disaster_date, $disaster_id);
