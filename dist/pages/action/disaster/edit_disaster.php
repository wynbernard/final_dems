...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (empty($disaster_id) || empty($disaster_type) || empty($disaster_date) || $disaster_level <= 0) {
	...
	$sql = "UPDATE disaster_table SET disaster_name = ?, date = ?, level = ? WHERE disaster_id = ?";
	$stmt = mysqli_prepare($conn, $sql);
	mysqli_stmt_bind_param($stmt, "ssii", $disaster_type, $disaster_date, $disaster_level, $disaster_id);
