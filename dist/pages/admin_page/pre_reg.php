...
WHERE (pre_reg_table.relation_to_family = 'Head of Family' OR pre_reg_table.registered_as = 'Solo')
";
 												...
 												<?php
													...
													?>
 														<tr>
 															...
 															<td>
 																<?php
																	// Pre-render family members content for pre_reg modal
																	$familyMembersHtml = '';
																	...
																	if ($familyId > 0) {
																		...
																		if ($headRes && mysqli_num_rows($headRes) > 0) {
																			...
																			<div class="mb-3">
																				...
																				<div class="row g-3">
																					...
																					<div class="col-md-6">
																						<p class="mb-1"><strong>Address:</strong> ' . htmlspecialchars($barangayName . ', ' . $district . ', ' . $city . ', ' . $province . ', ' . $region) . '</p>
																					</div>
																				</div>
																			</div>';
																		}
																		...
																		$dobObj = !empty($row['date_of_birth']) ? new DateTime($row['date_of_birth']) : null;
																		$ageSolo = $dobObj ? (new DateTime('today'))->diff($dobObj)->y : '';
																		$fullSolo = trim(($row['f_name'] ?? '') . ' ' . (!empty($row['m_name']) ? $row['m_name'] . ' ' : '') . ($row['l_name'] ?? ''));
																		$familyMembersHtml = '
																		<div class="container bg-white p-3">
																			<h6 class="fw-bold mb-2"><i class="fas fa-id-card me-2 text-primary"></i>Individual Details</h6>
																			<div class="row g-3">
																				<div class="col-md-6">
																					<p class="mb-1"><strong>Name:</strong> ' . htmlspecialchars($fullSolo) . '</p>
																					<p class="mb-1"><strong>Gender:</strong> ' . htmlspecialchars($row['gender'] ?? '') . '</p>
																					<p class="mb-1"><strong>Age:</strong> ' . htmlspecialchars($ageSolo) . '</p>
																				</div>
																				<div class="col-md-6">
																					<p class="mb-1"><strong>Address:</strong> ' . htmlspecialchars($barangayName . ', ' . $district . ', ' . $city . ', ' . $province . ', ' . $region) . '</p>
																				</div>
																	...
																	?>
 																<div class="d-none" id="family-members-pre-<?= (int)$row['pre_reg_id']; ?>"><?= $familyMembersHtml; ?></div>
 																<button
 		...
 		<div class="modal-content">
 			<div class="modal-header details-modal-header w-100">
 				<div class="w-100 d-flex align-items-center justify-content-between flex-wrap gap-2">
 					<h5 class="modal-title d-flex align-items-center gap-2 m-0" id="registrationModalLabel"><i class="fas fa-user-plus text-primary"></i> <span>Registration Form</span></h5>
 					<div class="d-flex align-items-center gap-2 flex-wrap" id="regModalChips"></div>
 				</div>
 ...
 <style>
 	/* Reuse details modal styling for consistency */
 	#registrationModal .details-modal-header,
 	#idCardModal .details-modal-header {
 		background: linear-gradient(90deg, rgba(13, 110, 253, 0.10) 0%, rgba(13, 110, 253, 0.02) 100%);
 		border-bottom: 1px solid #e9ecef;
 	}

 	#registrationModal .details-modal-body,
 	#idCardModal .details-modal-body {
 		background-color: #f8f9fb;
 	}

 	#registrationModal .details-card,
 	#idCardModal .details-card {
 		background: #fff;
 	...
 	#registrationModal .details-card+.details-card,
 	#idCardModal .details-card+.details-card {
 		margin-top: .75rem;
 	}
...
ORDER BY prt.registered_as, prt.family_id, prt.relation_to_family, prt.l_name, prt.f_name";
	...
	$soloCount = count(array_filter($evacuees, function ($e) {
		return $e['registered_as'] === 'Solo';
	}));
	$familyHeadCount = count(array_filter($evacuees, function ($e) {
		return $e['relation_to_family'] === 'Head of Family';
	}));
	?>
 ...
 <div class="modal fade" id="idCardModal" tabindex="-1" aria-labelledby="idCardModalLabel" aria-hidden="true">
 	<div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
 		<div class="modal-content">
 			<!-- Modal Header -->
 			<div class="modal-header details-modal-header w-100">
 				<h5 class="modal-title m-0 d-flex align-items-center gap-2" id="idCardModalLabel">
 							...
 							<div class="col-md-6">
 								<div class="id-card <?= ($evacuee['registered_as'] === 'Family') ? 'family-card' : 'solo-card' ?>">
 									...
 									<div class="card-header">
 										<div class="card-title">Evacuation System</div>
